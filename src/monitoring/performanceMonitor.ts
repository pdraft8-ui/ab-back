...
export const databaseMonitoringMiddleware = (
  ...
  if (originalQuery) {
    ...
    collections.forEach((collectionName) => {
      const collection =
        req.app.locals.mongoose?.connection?.db?.collection(collectionName);
      if (collection) {
        ...
        collection.find = function (...args: any[]) {
          const start = performance.now();
          ...
          result.toArray().then(() => {
            ...
            databaseQueryDuration.observe(
              { collection: collectionName, operation: "find" },
              duration / 1000
            );
        ...
        collection.findOne = function (...args: any[]) {
          const start = performance.now();
          ...
          result.then(() => {
            const duration = performance.now() - start;
            databaseQueryDuration.observe(
              { collection: collectionName, operation: "findOne" },
              duration / 1000
            );
            databaseQueryTotal.inc({
              collection: collectionName,
        ...
        collection.insertOne = function (...args: any[]) {
          const start = performance.now();
          ...
          result.then(() => {
            const duration = performance.now() - start;
            databaseQueryDuration.observe(
              { collection: collectionName, operation: "insertOne" },
              duration / 1000
            );
            databaseQueryTotal.inc({
              collection: collectionName,
        ...
        collection.updateOne = function (...args: any[]) {
          const start = performance.now();
          ...
          result.then(() => {
            const duration = performance.now() - start;
            databaseQueryDuration.observe(
              { collection: collectionName, operation: "updateOne" },
              duration / 1000
            );
            databaseQueryTotal.inc({
              collection: collectionName,
        ...
        collection.deleteOne = function (...args: any[]) {
          const start = performance.now();
          ...
          result.then(() => {
            const duration = performance.now() - start;
            databaseQueryDuration.observe(
              { collection: collectionName, operation: "deleteOne" },
              duration / 1000
            );
            databaseQueryTotal.inc({
              collection: collectionName,
...
export const collectSystemMetrics = (): SystemMetrics => {
...
export const updateCacheMetrics = (hitCount: number, missCount: number) => {
...
const generateRecommendations = (metrics: SystemMetrics): string[] => {
  ...
  if (metrics.memory.percentage > 80) {
    recommendations.push(
      "High memory usage detected. Consider optimizing memory usage or scaling up."
    );
  }
