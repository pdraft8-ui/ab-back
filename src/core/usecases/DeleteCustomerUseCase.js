  ...
  async execute(customerId, userId, userName) {
    try {
      ...
      if (existingCustomer.vehicles && existingCustomer.vehicles.length > 0) {
        throw new Error(
          "Cannot delete customer with vehicles. Please remove vehicles first."
        );
      }

      if (
        existingCustomer.insurances &&
        existingCustomer.insurances.length > 0
      ) {
        throw new Error(
          "Cannot delete customer with insurances. Please remove insurances first."
        );
      }
