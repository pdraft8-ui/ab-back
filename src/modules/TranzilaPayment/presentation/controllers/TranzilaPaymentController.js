...
export class TranzilaPaymentController {
  ...
  async createDirectPayment(req, res, next) {
    try {
      ...
      const result = await this.createTranzilaPaymentUseCase.execute(
        paymentData,
        userId
      );
  ...
  async getPaymentStatus(req, res, next) {
    try {
      ...
      const payment =
        await this.paymentRepository.findTranzilaPaymentByPaymentId(paymentId);
      if (!payment) {
      ...
      if (payment.isPending() && payment.hasTranzilaTransactionId()) {
        const statusResult = await this.tranzilaService.checkPaymentStatus(
          payment.tranzilaTransactionId
        );
  ...
  async refundPayment(req, res, next) {
    try {
      ...
      const refundResult = await this.tranzilaService.processRefund(
        payment.tranzilaTransactionId,
        refundAmount,
        reason
      );
  ...
  async getPaymentHistory(req, res, next) {
    try {
      ...
      const payments = await this.paymentRepository.getAllTranzilaPayments(
        filters
      );
