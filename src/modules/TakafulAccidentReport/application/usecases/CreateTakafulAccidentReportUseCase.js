...
export class CreateTakafulAccidentReportUseCase {
  constructor(
    takafulAccidentReportRepository,
    customerRepository,
    auditService,
    notificationService
  ) {
  ...
  async execute(accidentReportData, plateNumber, userId, userName) {
    try {
      ...
      if (!vehicle) {
        throw new Error(
          "Vehicle not found under the customer person's vehicles"
        );
      }
      ...
      const createdAccidentReport =
        await this.takafulAccidentReportRepository.create(
          accidentReport.toJSON()
        );
      ...
      await this.auditService.logAction(
        "CREATE_TAKAFUL_ACCIDENT_REPORT",
        `Created takaful accident report for customer: ${customer.name}`,
        userId,
        userName,
        null,
        createdAccidentReport
      );
