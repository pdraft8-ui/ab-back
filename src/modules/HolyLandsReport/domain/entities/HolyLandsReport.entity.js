export class HolyLandsReport {
  ...
  isValidInsuranceDetails() {
    if (!this.insuranceDetails) return true;
  ...
  isValidVehicleDetails() {
    if (!this.vehicleDetails) return true;
  ...
  isValidOwnerAndDriverDetails() {
    if (!this.ownerAndDriverDetails) return true;
  ...
  isValidAccidentDetails() {
    ...
    const { accidentDate, numberOfPassengers, policeNotified } =
  ...
  isValidOtherVehicles() {
    if (!Array.isArray(this.otherVehicles)) return false;
  ...
  isValidInvolvementDetails() {
    if (!this.involvementDetails) return true;
  ...
  isValidInjuries() {
    ...
    return this.injuries.every((injury) => {
      const { name, age, address, occupation, maritalStatus, injuryType } =
        injury;
