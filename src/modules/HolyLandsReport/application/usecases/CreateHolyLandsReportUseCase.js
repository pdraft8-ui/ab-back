...
export class CreateHolyLandsReportUseCase {
  ...
  async execute(accidentData, plateNumber, userId, userName) {
    try {
      ...
      const customer = await this.customerRepository.findByPlateNumber(
        plateNumber
      );
      if (!customer) {
      ...
      const accidentReportData = {
        ...
        ownerAndDriverDetails: {
          ...
          driverLicenseNumber:
            accidentData.ownerAndDriverDetails?.driverLicenseNumber || "",
          driverLicenseGrade:
            accidentData.ownerAndDriverDetails?.driverLicenseGrade || "",
          ...
          driverAddress:
            accidentData.ownerAndDriverDetails?.driverAddress || "",
          driverProfession:
            accidentData.ownerAndDriverDetails?.driverProfession || "",
          licenseIssuePlace:
            accidentData.ownerAndDriverDetails?.licenseIssuePlace || "",
        },
        ...
        passengerNamesAndAddresses:
          accidentData.passengerNamesAndAddresses || "",
      ...
      // Save to repository
      const savedAccidentReport = await this.holyLandsReportRepository.create(
        accidentReport
      );
