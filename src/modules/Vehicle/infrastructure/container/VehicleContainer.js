...
export class VehicleContainer {
  constructor() {
    ...
    this.createVehicleUseCase = new CreateVehicleUseCase(
      this.vehicleRepository,
      this.customerRepository,
      this.auditService
    );

    this.getAllVehiclesUseCase = new GetAllVehiclesUseCase(
      this.vehicleRepository
    );

    this.getVehicleByIdUseCase = new GetVehicleByIdUseCase(
      this.vehicleRepository
    );

    this.updateVehicleUseCase = new UpdateVehicleUseCase(
      this.vehicleRepository,
      this.auditService
    );

    this.deleteVehicleUseCase = new DeleteVehicleUseCase(
      this.vehicleRepository,
      this.auditService
    );

    this.getVehiclesByCustomerUseCase = new GetVehiclesByCustomerUseCase(
      this.vehicleRepository,
      this.customerRepository
    );

    this.addInsuranceToVehicleUseCase = new AddInsuranceToVehicleUseCase(
      this.vehicleRepository,
      this.auditService
    );

    this.getVehicleStatsUseCase = new GetVehicleStatsUseCase(
      this.vehicleRepository
    );
    ...
    this.vehicleController = new VehicleController(
      ...
      this.getVehiclesByCustomerUseCase,
      this.addInsuranceToVehicleUseCase,
      this.getVehicleStatsUseCase
    );
...
export class VehicleContainerMinimal {
  constructor() {
    this.mockVehicleRepository = {
      ...
      delete: async (id) =>
        new Vehicle({ id, plateNumber: 12345, model: "Deleted Vehicle" }),
      addInsuranceToVehicle: async (vehicleId, insuranceData) =>
        new Vehicle({ id: vehicleId, insurance: [insuranceData] }),
      getStats: async () => ({
        totalVehicles: 2,
