...
export class VehicleController {
  ...
  async getAllVehicles(req, res, next) {
    try {
      ...
      const vehiclesWithUrls = vehicles.map((vehicle) =>
        this.convertVehicleToFullUrls(vehicle)
      );
  ...
  async getVehiclesByCustomer(req, res, next) {
    try {
      ...
      const vehicles = await this.getVehiclesByCustomerUseCase.execute(
        customerId
      );
      const vehiclesWithUrls = vehicles.map((vehicle) =>
        this.convertVehicleToFullUrls(vehicle)
  ...
  async addInsuranceToVehicle(req, res, next) {
    try {
      ...
      const vehicle = await this.addInsuranceToVehicleUseCase.execute(
        vehicleId,
        insuranceData,
        userId,
        userName
