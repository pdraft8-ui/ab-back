...
export class UpdateInsuranceCompanyUseCase {
  ...
  async execute(id, updateData, userId, userName) {
    try {
      ...
      const existingCompany = await this.insuranceCompanyRepository.findById(
        id
      );
      if (!existingCompany) {
      ...
      if (!company.isValid()) {
        throw new Error(
          "Invalid insurance company data. Name and insurance type are required."
        );
      }
      ...
      if (!company.isMandatoryInsurance() && !company.hasRates()) {
        throw new Error(
          "Rate details are required for comprehensive or third-party insurance."
        );
      }
      ...
      if (updateData.name && updateData.name !== existingCompany.name) {
        const companyWithSameName =
          await this.insuranceCompanyRepository.findByName(updateData.name);
        if (companyWithSameName && companyWithSameName.id !== id) {
          throw new Error(
            `Insurance company with name '${updateData.name}' already exists.`
          );
        }
      }
      ...
      // Update the company
      const updatedCompany = await this.insuranceCompanyRepository.update(
        id,
        updateData
      );
