...
export class CreateInsuranceCompanyUseCase {
  ...
  async execute(companyData, userId, userName) {
    try {
      ...
      const existingCompany = await this.insuranceCompanyRepository.findByName(
        company.name
      );
      if (existingCompany) {
        throw new Error(
          `Insurance company with name '${company.name}' already exists.`
        );
      }

      // Create the company
      const createdCompany = await this.insuranceCompanyRepository.create(
        companyData
      );
