export class InsuranceCompany {
  ...
  addRate(rateType, rateData) {
    if (!this.isValidRateData(rateData)) {
      throw new Error(
        "Invalid rate data. All required fields must be provided."
      );
    }
  ...
  updateCompanyInfo(companyInfo) {
    if (companyInfo.name) this.name = companyInfo.name;
    if (companyInfo.contact !== undefined) this.contact = companyInfo.contact;
    if (companyInfo.address !== undefined) this.address = companyInfo.address;
