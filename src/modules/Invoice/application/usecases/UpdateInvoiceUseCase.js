...
export class UpdateInvoiceUseCase {
  ...
  async execute(invoiceId, updateData, userId, userName) {
    try {
      ...
      const existingInvoice = await this.invoiceRepository.getInvoiceById(
        invoiceId
      );
      if (!existingInvoice) {
      ...
      if (updateData.totalAmount !== undefined) {
        ...
        existingInvoice.balanceDue = Math.max(
          0,
          updateData.totalAmount - paidAmount
        );
      }
  ...
  validateUpdateData(updateData) {
    ...
    if (
      ...
      !["Pending", "Partially Paid", "Paid", "Overdue"].includes(
        updateData.status
      )
    ) {
