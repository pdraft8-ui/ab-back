...
export class DocumentSettingsContainer {
  constructor() {
    ...
    this.createDocumentSettingsUseCase = new CreateDocumentSettingsUseCase(
      this.documentSettingsRepository,
      this.auditService
    );

    this.getAllDocumentSettingsUseCase = new GetAllDocumentSettingsUseCase(
      this.documentSettingsRepository
    );

    this.getDocumentSettingsByIdUseCase = new GetDocumentSettingsByIdUseCase(
      this.documentSettingsRepository
    );
    ...
    this.updateDocumentSettingsUseCase = new UpdateDocumentSettingsUseCase(
      this.documentSettingsRepository,
      this.auditService
    );

    this.deleteDocumentSettingsUseCase = new DeleteDocumentSettingsUseCase(
      this.documentSettingsRepository,
      this.auditService
    );

    this.toggleDocumentSettingsStatusUseCase =
      new ToggleDocumentSettingsStatusUseCase(
        this.documentSettingsRepository,
        this.auditService
      );

    this.uploadHeaderLogoUseCase = new UploadHeaderLogoUseCase(
      this.documentSettingsRepository,
      this.auditService
    );

    this.uploadFooterLogoUseCase = new UploadFooterLogoUseCase(
      this.documentSettingsRepository,
      this.auditService
    );

    this.uploadSignatureUseCase = new UploadSignatureUseCase(
      this.documentSettingsRepository,
      this.auditService
    );

    this.getDocumentSettingsStatsUseCase = new GetDocumentSettingsStatsUseCase(
      this.documentSettingsRepository
    );

    // Initialize controller
    this.documentSettingsController = new DocumentSettingsController(
      this.createDocumentSettingsUseCase,
      this.getAllDocumentSettingsUseCase,
      this.getDocumentSettingsByIdUseCase,
      this.getDocumentSettingsByTypeUseCase,
      this.updateDocumentSettingsUseCase,
      this.deleteDocumentSettingsUseCase,
      this.toggleDocumentSettingsStatusUseCase,
      this.uploadHeaderLogoUseCase,
      this.uploadFooterLogoUseCase,
      this.uploadSignatureUseCase,
      this.getDocumentSettingsStatsUseCase
    );

    // Initialize routes
    this.documentSettingsRoutes = new DocumentSettingsRoutes(
  ...
  constructor() {
    this.mockDocumentSettingsRepository = {
      ...
      toggleStatus: async (id) =>
        new DocumentSettings({ id, isActive: false, createdBy: "mock-user" }),
      updateHeaderLogo: async (id, logoData) =>
        new DocumentSettings({
          id,
          header: { logo: logoData },
          createdBy: "mock-user",
        }),
      updateFooterLogo: async (id, logoData) =>
        new DocumentSettings({
          id,
