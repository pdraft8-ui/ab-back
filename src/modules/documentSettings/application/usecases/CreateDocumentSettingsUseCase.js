...
export class CreateDocumentSettingsUseCase {
  ...
  async execute(settingsData, userId, userName) {
    try {
      ...
      if (!settings.isValid()) {
        throw new Error(
          "Invalid document settings data. Document type and creator are required."
        );
      }

      // Check if settings already exist for this document type
      const existingSettings =
        await this.documentSettingsRepository.findByDocumentType(
          settings.documentType
        );
      if (existingSettings) {
        throw new Error(
          `Document settings for ${settings.documentType} already exist`
        );
      }
