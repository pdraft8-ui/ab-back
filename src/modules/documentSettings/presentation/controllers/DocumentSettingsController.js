...
export class DocumentSettingsController {
  constructor(
    createDocumentSettingsUseCase,
    getAllDocumentSettingsUseCase,
    getDocumentSettingsByIdUseCase,
    getDocumentSettingsByTypeUseCase,
    updateDocumentSettingsUseCase,
    deleteDocumentSettingsUseCase,
    toggleDocumentSettingsStatusUseCase,
    uploadHeaderLogoUseCase,
    uploadFooterLogoUseCase,
    uploadSignatureUseCase,
    getDocumentSettingsStatsUseCase
  ) {
  ...
  convertDocumentSettingsToFullUrls(settings) {
    ...
    if (settingsObj.header?.logo?.url) {
      settingsObj.header.logo.url = localStorageService.getFullUrl(
    ...
    if (settingsObj.footer?.logo?.url) {
      settingsObj.footer.logo.url = localStorageService.getFullUrl(
    ...
    if (settingsObj.footer?.signature?.url) {
      settingsObj.footer.signature.url = localStorageService.getFullUrl(
  ...
  async createDocumentSettings(req, res, next) {
    try {
      ...
      const settings = await this.createDocumentSettingsUseCase.execute(
        settingsData,
        userId,
        userName
      );
  ...
  async getAllDocumentSettings(req, res, next) {
    try {
      ...
      const settingsWithUrls = settings.map((setting) =>
        this.convertDocumentSettingsToFullUrls(setting)
      );
  ...
  async getDocumentSettingsByType(req, res, next) {
    try {
      ...
      const settings = await this.getDocumentSettingsByTypeUseCase.execute(
        documentType
      );
      const settingsWithUrls = this.convertDocumentSettingsToFullUrls(settings);
  ...
  async updateDocumentSettings(req, res, next) {
    try {
      ...
      const settings = await this.updateDocumentSettingsUseCase.execute(
        id,
        settingsData,
        userId,
        userName
  ...
  async deleteDocumentSettings(req, res, next) {
    try {
      ...
      const result = await this.deleteDocumentSettingsUseCase.execute(
        id,
        userId,
        userName
      );
  ...
  async toggleDocumentSettingsStatus(req, res, next) {
    try {
      ...
      const settings = await this.toggleDocumentSettingsStatusUseCase.execute(
        id,
        userId,
        userName
      );
  ...
  async uploadHeaderLogo(req, res, next) {
    try {
      ...
      const settings = await this.uploadHeaderLogoUseCase.execute(
        documentType,
        logoData,
        userId,
        userName
  ...
  async uploadFooterLogo(req, res, next) {
    try {
      ...
      const settings = await this.uploadFooterLogoUseCase.execute(
        documentType,
        logoData,
        userId,
        userName
  ...
  async uploadSignature(req, res, next) {
    try {
      ...
      const settings = await this.uploadSignatureUseCase.execute(
        documentType,
        signatureData,
        userId,
        userName
