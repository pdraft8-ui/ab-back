...
export const addNewAccedentReport = async (req, res) => {
  ...
  try {
    ...
    const addNewAccedentReport = new AhliAccidentReportModel({
      ...
      customerPerson: {
        name: customer.fullName || req.body.customerPerson.name,
      },
      ...
      vehicleInfo: {
        ...
        licenseExpiry:
          vehicle.licenseExpiry || req.body.vehicleInfo.licenseExpiry,
      },
...
export const deleteAccidentReport = async (req, res) => {
  try {
    ...
    if (!deleteAcc) {
      return res
        .status(400)
        .json({ message: "Accident report deletion failed." });
    }
...
export const getAllAccidentReports = async (req, res, next) => {
  try {
    const allReports = await AhliAccidentReportModel.find();
    return res.status(200).json({
      success: true,
      message: "Success",
      data: allReports || [],
