...
import { getRecordingPath } from "../../../Servicess/recording.service.js";
...
export const getCallRecording = async (req, res) => {
  try {
    ...
    if (!callid || !token_id) {
      return res
        .status(400)
        .json({ message: "callid and token_id are required" });
    }
    ...
    if (!recordingUrl) {
      return res
        .status(404)
        .json({ message: "Recording not found or request timed out" });
    }
