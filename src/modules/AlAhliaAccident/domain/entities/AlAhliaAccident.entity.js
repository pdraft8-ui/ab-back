export class AlAhliaAccident {
  ...
  isValidPolicyInfo() {
    if (!this.policyInfo) return true;
  ...
  isValidCustomerPerson() {
    if (!this.customerPerson) return true;
    return typeof this.customerPerson.name === "string";
  }

  isValidDriverInfo() {
    if (!this.driverInfo) return true;
    const {
      name,
      idNumber,
      age,
  ...
  isValidVehicleInfo() {
    if (!this.vehicleInfo) return true;
    const {
      usage,
      manufactureYear,
      vehicleType,
  ...
  isValidAccidentDetails() {
    if (!this.accidentDetails) return true;
    const {
      location,
      time,
      weather,
  ...
  isValidThirdPartyVehicles() {
    if (!Array.isArray(this.thirdPartyVehicles)) return false;
  ...
  isValidThirdPartyInjuries() {
    if (!Array.isArray(this.thirdPartyInjuries)) return false;

    return this.thirdPartyInjuries.every((injury) => {
      return (
        typeof injury.name === "string" &&
  ...
  isValidThirdPartyPassengers() {
    if (!Array.isArray(this.thirdPartyPassengers)) return false;
  ...
  isValidExternalWitnesses() {
    if (!Array.isArray(this.externalWitnesses)) return false;
  ...
  isValidDeclaration() {
    ...
    const { driverSignature, declarationDate, officerSignature, officerDate } =
      this.declaration;
