...
const insuranceFilesDir = path.join(process.cwd(), "uploads/Customer/InsuranceFiles/");
...
export function myMulter(customValidation = null, destination = "uploads/general", maxFileSize = MAX_FILE_SIZE, allowedTypes = ALLOWED_ALL_TYPES) {
    ...
    function fileFilter(req, file, cb) {
        ...
        if (file.size > maxFileSize) {
            return cb(new Error(`File size exceeds maximum allowed size of ${maxFileSize / (1024 * 1024)}MB`), false);
        }
        // Check file type
...
export function myMulterImages(destination = "uploads/images") {
    return myMulter(ALLOWED_IMAGE_TYPES, destination, MAX_IMAGE_SIZE, ALLOWED_IMAGE_TYPES);
}
export function myMulterDocuments(destination = "uploads/documents") {
    return myMulter(ALLOWED_DOCUMENT_TYPES, destination, MAX_DOCUMENT_SIZE, ALLOWED_DOCUMENT_TYPES);
}
export function myMulterAllFiles(destination = "uploads/general") {
    return myMulter(ALLOWED_ALL_TYPES, destination, MAX_FILE_SIZE, ALLOWED_ALL_TYPES);
}
export function myMulterInsuranceFiles(destination = "uploads/Customer/InsuranceFiles") {
    return myMulter(ALLOWED_DOCUMENT_TYPES, destination, MAX_DOCUMENT_SIZE, ALLOWED_DOCUMENT_TYPES);
}
...
export const validateUploadedFiles = (req, res, next) => {
    ...
    try {
        ...
        for (const file of files) {
            ...
            const validatedFile = validateAndSanitizeFile(file, ALLOWED_ALL_TYPES, MAX_FILE_SIZE);
            if (!validatedFile) {
                return res.status(400).json({
                    success: false,
