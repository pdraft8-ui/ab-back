...
class EmailService {
    ...
    initTransporter() {
        try {
            ...
            if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
                logger.warn("Gmail configuration missing. Email service will not work.");
                return;
            }
    ...
    async fetchInboxEmails(options = {}) {
        const { limit = 50, offset = 0, search = "", from = "", to = "", subject = "", dateFrom = "", dateTo = "", unreadOnly = false, } = options;
        if (!this.imapConfig) {
            throw new Error("IMAP service not configured");
        }
        return new Promise((resolve, reject) => {
            ...
            imap.once("ready", () => {
                imap.openBox("INBOX", false, (err, box) => {
                    ...
                    const finalCriteria = searchCriteria.length > 0 ? searchCriteria : ["ALL"];
    ...
    async sendEmail(options) {
        const { to, subject, html, text, attachments = [], cc = [], bcc = [], replyTo = null, } = options;
        if (!this.isConfigured) {
            throw new Error("Email service not configured. Please check GMAIL_USER and GMAIL_APP_PASSWORD environment variables.");
        }
        try {
            const mailOptions = {
                from: `"${process.env.EMAIL_FROM_NAME || "Insurance Management System"}" <${process.env.GMAIL_USER}>`,
          ...
          <div class="container">
            ...
            <div class="content">
              ...
              <div class="invoice-details">
                ...
                <p class="amount"><strong>Amount:</strong> $${invoiceData.totalAmount.toFixed(2)}</p>
              </div>
          ...
          <div class="container">
            ...
            <div class="content">
              ...
              <div class="payment-details">
                ...
                <p class="amount"><strong>Amount Paid:</strong> $${paymentData.amount.toFixed(2)}</p>
              </div>
...
export const sendPasswordResetEmail = (email, resetCode, userName) => emailService.sendPasswordResetEmail(email, resetCode, userName);
export const sendWelcomeEmail = (email, userName) => emailService.sendWelcomeEmail(email, userName);
export const sendInvoiceNotification = (email, invoiceData, customerName) => emailService.sendInvoiceNotification(email, invoiceData, customerName);
export const sendPaymentConfirmation = (email, paymentData, customerName) => emailService.sendPaymentConfirmation(email, paymentData, customerName);
...
export const fetchInboxEmails = (options) => emailService.fetchInboxEmails(options);
export const getEmailByUid = (uid) => emailService.getEmailByUid(uid);
export const markEmailAsRead = (uid, read) => emailService.markEmailAsRead(uid, read);
