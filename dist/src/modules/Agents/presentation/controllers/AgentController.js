export class AgentController {
    constructor(createAgentUseCase, getAllAgentsUseCase, getAgentByIdUseCase, updateAgentUseCase, deleteAgentUseCase, getAgentStatsUseCase) {
    ...
    async getAllAgents(req, res) {
        try {
            ...
            const result = await this.getAllAgentsUseCase.execute(filters, pagination);
            if (result.success) {
    ...
    async updateAgent(req, res) {
        try {
            ...
            const userId = req.user?.id || "system";
            const result = await this.updateAgentUseCase.execute(id, updateData, userId);
            if (result.success) {
                return res.status(200).json({
                    success: true,
                    message: result.message,
