    ...
    async execute(agentId, updateData, userId) {
        try {
            ...
            if (updateData.email && updateData.email !== existingAgent.getEmail()) {
                const existingAgentByEmail = await this.agentRepository.findByEmail(updateData.email);
            ...
            if (updateData.phone && updateData.phone !== existingAgent.getPhone()) {
                const existingAgentByPhone = await this.agentRepository.findByPhone(updateData.phone);
            ...
            const savedAgent = await this.agentRepository.update(agentId, updatedAgent);
            // Log audit
...
//# sourceMappingURL=UpdateAgentUseCase.js.map