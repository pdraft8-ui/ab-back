export class GetAgentStatsUseCase {
    ...
    async execute() {
        try {
            ...
            const recentAgents = allAgents.filter((agent) => new Date(agent.getCreatedAt()) >= thirtyDaysAgo);
            // Calculate agents created in last 7 days
            const sevenDaysAgo = new Date();
            sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
            const weeklyAgents = allAgents.filter((agent) => new Date(agent.getCreatedAt()) >= sevenDaysAgo);
            // Calculate agents created today
            const today = new Date();
            today.setHours(0, 0, 0, 0);
            const todayAgents = allAgents.filter((agent) => new Date(agent.getCreatedAt()) >= today);
            ...
            const growthRate = lastMonthAgents.length > 0
...
//# sourceMappingURL=GetAgentStatsUseCase.js.map