...
export const addVehicle = async (req, res, next) => {
    const { customerId: bodyCustomerId, plateNumber, model, type, ownership, modelNumber, licenseExpiry, lastTest, color, price, } = req.body;
    ...
    try {
        ...
        let imageUrl = null;
        if (req.file) {
            const result = await localStorageService.upload(req.file, "vehicles/images");
            imageUrl = result.url;
        }
        else {
            imageUrl =
...
export const allVehicles = async (req, res, next) => {
    try {
        const vehicles = await Vehicle.find().populate("customerId", "first_name last_name");
        // Convert all vehicles to include full URLs
        const vehiclesWithFullUrls = vehicles.map((vehicle) => convertVehicleToFullUrls(vehicle));
...
export const findVehicleById = async (req, res, next) => {
    try {
        const { id } = req.params;
        const vehicle = await Vehicle.findById(id).populate("customerId", "first_name last_name");
        if (!vehicle) {
            return res.status(404).json({ message: "Vehicle not found" });
        }
...
export const findVehicleByIdUnified = async (req, res, next) => {
    try {
        ...
        let vehicle = await Vehicle.findById(id).populate("customerId", "first_name last_name phone_number email");
        if (vehicle) {
            return res.status(200).json({
...
export const getVehiclesByCustomer = async (req, res, next) => {
    try {
        ...
        const vehicles = await Vehicle.find({ customerId }).populate("customerId", "first_name last_name");
...
export const updateVehicle = async (req, res, next) => {
    try {
        ...
        const updatedVehicle = await Vehicle.findByIdAndUpdate(id, {
            ...
            image: imageUrl,
        }, { new: true }).populate("customerId", "first_name last_name");
        const findUser = await UserModel.findById(req.user._id);
    ...
    try {
        ...
        if (req.files && req.files.length > 0) {
            for (const file of req.files) {
                const result = await localStorageService.upload(file, "vehicles/insurance-files");
    ...
    try {
        ...
        if (customer) {
            ...
            if (vehicleSubdoc) {
                ...
                if (req.files && req.files.length > 0) {
                    for (const file of req.files) {
                        const result = await localStorageService.upload(file, "customers/insurance-files");
                        insuranceFilesUrls.push(result.url);
                    }
                }
                // Map Arabic insurance types to English
                ...
                const findUser = await UserModel.findById(req.user._id);
                await customer.save();
                const addedInsurance = vehicleSubdoc.insurance[vehicleSubdoc.insurance.length - 1];
                // Create invoice automatically
                const remainingAmount = insuranceAmount - (paidAmount || 0);
                if (remainingAmount > 0) {
                    const newInvoice = new Invoice({
...
export const removeInsuranceFromVehicle = async (req, res, next) => {
    try {
        ...
        const insuranceIndex = findVehicle.insurance.findIndex((insurance) => insurance._id.toString() === insuranceId);
        if (insuranceIndex === -1) {
...
export const addCheckToInsurance = async (req, res, next) => {
    try {
        ...
        const insurance = findVehicle.insurance.find((ins) => ins._id.toString() === insuranceId);
        if (!insurance) {
            return res.status(404).json({ message: "Insurance not found" });
        }
        let checkImage = null;
        if (req.file) {
            const result = await localStorageService.upload(req.file, "vehicles/checks");
