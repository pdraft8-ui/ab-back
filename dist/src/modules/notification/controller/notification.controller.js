...
export const sendNotificationLogic = async ({ senderId, message }) => {
    ...
    if (sender.role === "employee") {
        ...
        if (departmentHead)
            recipients.push(departmentHead._id);
    }
    const admins = await User.find({ role: "admin" }).lean();
    recipients.push(...admins.map((admin) => admin._id));
    recipients = [...new Set(recipients.map((id) => id.toString()))];
    const notifications = await Promise.all(recipients.map(async (recipientId) => {
...
export const markAsRead = async (req, res) => {
    try {
        ...
        const userId = req.user.id;
        const notification = await notificationModel.findOneAndUpdate({ _id: notificationId, recipient: userId }, { isRead: true }, { new: true });
        if (!notification) {
