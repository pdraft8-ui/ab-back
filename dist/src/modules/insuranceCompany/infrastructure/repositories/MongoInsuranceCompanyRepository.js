...
export class MongoInsuranceCompanyRepository extends IInsuranceCompanyRepository {
    ...
    async findAll() {
        try {
            ...
            return companies.map((company) => this.mapToInsuranceCompanyEntity(company));
        }
        catch (error) {
    ...
    async update(id, companyData) {
        try {
            const company = await InsuranceCompanyModel.findByIdAndUpdate(id, { ...companyData, updatedAt: new Date() }, { new: true });
            return this.mapToInsuranceCompanyEntity(company);
        }
        catch (error) {
    ...
    async addRate(id, rateType, rateData) {
        try {
            const company = await InsuranceCompanyModel.findByIdAndUpdate(id, { [`rates.${rateType}`]: rateData, updatedAt: new Date() }, { new: true });
            return this.mapToInsuranceCompanyEntity(company);
        }
        catch (error) {
    ...
    async removeRate(id, rateType) {
        try {
            const company = await InsuranceCompanyModel.findByIdAndUpdate(id, { $unset: { [`rates.${rateType}`]: 1 }, updatedAt: new Date() }, { new: true });
            return this.mapToInsuranceCompanyEntity(company);
        }
        catch (error) {
    ...
    async getStats() {
        try {
            ...
            const comprehensiveCompanies = await InsuranceCompanyModel.countDocuments({ insuranceType: "thirdPartyComprehensive" });
...
//# sourceMappingURL=MongoInsuranceCompanyRepository.js.map