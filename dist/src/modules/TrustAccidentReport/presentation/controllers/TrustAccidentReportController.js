export class TrustAccidentReportController {
    constructor(createTrustAccidentReportUseCase, getAllTrustAccidentReportsUseCase, getTrustAccidentReportByIdUseCase, deleteTrustAccidentReportUseCase, getTrustAccidentReportStatsUseCase) {
    ...
    async addAccidentReport(req, res) {
        try {
            ...
            const user = req.user;
            const result = await this.createTrustAccidentReportUseCase.execute(accidentReportData, plateNumber, user);
            ...
            return res.status(500).json({
                message: error.message || "An error occurred while adding the accident report",
            });
        }
    }
    async getAllAccidentReports(req, res) {
        try {
            const accidentReports = await this.getAllTrustAccidentReportsUseCase.execute();
    ...
    async findById(req, res) {
        try {
            const { id } = req.params;
            const accidentReport = await this.getTrustAccidentReportByIdUseCase.execute(id);
            return res.status(200).json({
                message: "Success",
    ...
    async deleteAccidentReport(req, res) {
        try {
            const { id } = req.params;
            const user = req.user;
            const result = await this.deleteTrustAccidentReportUseCase.execute(id, user);
            return res.status(200).json({
...
//# sourceMappingURL=TrustAccidentReportController.js.map