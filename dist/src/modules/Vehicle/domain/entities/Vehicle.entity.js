export class Vehicle {
    ...
    hasExpiredTest() {
        ...
        const oneYearFromTest = new Date(testDate.getTime() + 365 * 24 * 60 * 60 * 1000);
    ...
    removeInsurance(insuranceId) {
        ...
        this.insurance = this.insurance.filter((insurance) => insurance.id !== insuranceId);
        return this.insurance.length < initialLength;
    }
    updateInsurance(insuranceId, insuranceData) {
        if (!this.insurance)
            return false;
        const index = this.insurance.findIndex((insurance) => insurance.id === insuranceId);
        if (index === -1)
            return false;
    ...
    getTotalInsuranceAmount() {
        if (!this.hasInsurance())
            return 0;
        return this.insurance.reduce((total, insurance) => total + (insurance.insuranceAmount || 0), 0);
    }
    getTotalPaidAmount() {
        if (!this.hasInsurance())
            return 0;
        return this.insurance.reduce((total, insurance) => total + (insurance.paidAmount || 0), 0);
    }
    getTotalRemainingDebt() {
        if (!this.hasInsurance())
            return 0;
        return this.insurance.reduce((total, insurance) => total + (insurance.remainingDebt || 0), 0);
    }
    toJSON() {
        return {
            id: this.id,
