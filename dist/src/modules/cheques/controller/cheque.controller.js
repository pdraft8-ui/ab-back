...
export const createCheque = async (req, res, next) => {
    try {
        const { customer, chequeNumber, chequeDate, chequeAmount, chequeStatus = "pending", bankName, accountNumber, notes, clearedDate, } = req.body;
        ...
        const newCheque = new chequeModel({
            ...
            clearedDate: chequeStatus === "cleared" ? clearedDate || new Date() : null,
...
export const getAllCheques = async (req, res, next) => {
    try {
        const { customer, customerName, customerPhone, chequeStatus, startDate, endDate, minAmount, maxAmount, bankName, page = 1, limit = 10, sortBy = "createdAt", sortOrder = "desc", } = req.query;
        ...
        if (error.name === "MongooseError" && error.message.includes("timeout")) {
            return res.status(408).json({
                message: "Request timeout - please try with fewer results or different filters",
                error: "TIMEOUT",
            });
        }
...
export const bulkUpdateChequeStatus = async (req, res, next) => {
    try {
        ...
        const result = await chequeModel.updateMany({ _id: { $in: chequeIds } }, updateData);
        // Get updated cheques
...
export const getCustomerCheques = async (req, res, next) => {
    try {
        ...
        const { chequeStatus, startDate, endDate, minAmount, maxAmount, page = 1, limit = 10, sortBy = "createdAt", sortOrder = "desc", } = req.query;
