...
import { sendPasswordResetEmail, verifyImapConnection, fetchInboxEmails, getEmailByUid, markEmailAsRead, deleteEmail, } from "../../../Servicess/email.js";
import { sendSingleSMS as sendSingleSMSService, sendBulkSMS as sendBulkSMSService, testSMSConfiguration as testSMSConfigurationService, getSMSServiceStatus, } from "../../../Servicess/sms.js";
...
export const sendCode = async (req, res, next) => {
    try {
        ...
        const code = nanoid();
        const user = await UserModel.findOneAndUpdate({ _id: findUser.id }, { sendCode: code }, { new: true });
        if (!user) {
...
export const updateEmployee = async (req, res, next) => {
    ...
    try {
        ...
        if (employee.departmentId) {
            ...
            if (department) {
                const employeeIndex = department.employees.findIndex((emp) => emp._id.toString() === employeeId);
                if (employeeIndex !== -1) {
...
export const testEmailConfiguration = async (req, res, next) => {
    try {
        const { verifyEmailConnection } = await import("../../../Servicess/email.js");
        // Test email connection
        const isConnected = await verifyEmailConnection();
        if (!isConnected) {
            return res.status(500).json({
                message: "Email service not configured properly",
                error: "Please check GMAIL_USER and GMAIL_APP_PASSWORD environment variables",
            });
        }
      ...
      `,
        ...
        return res.status(500).json({
            ...
            error: error.message,
            details: "Please check the EMAIL_CONFIGURATION.md file for setup instructions",
        });
    }
};
...
export const sendCustomEmail = async (req, res, next) => {
    try {
        const { to, subject, html, text, cc = [], bcc = [], attachments = [], replyTo = null, } = req.body;
...
export const sendBulkEmails = async (req, res, next) => {
    try {
        const { emails, subject, html, text, cc = [], bcc = [], attachments = [], } = req.body;
        // Validate required fields
...
export const getInboxEmails = async (req, res, next) => {
    try {
        const { limit = 50, page = 1, search = "", from = "", to = "", subject = "", dateFrom = "", dateTo = "", unreadOnly = false, } = req.query;
...
export const sendBulkSMS = async (req, res, next) => {
    try {
        ...
        console.log("🔍 Validation check - recipientsArray type:", typeof recipientsArray);
        console.log("🔍 Validation check - is array:", Array.isArray(recipientsArray));
        console.log("🔍 Validation check - length:", recipientsArray ? recipientsArray.length : "undefined");
        if (!recipientsArray) {
        ...
        const phoneRegex = /^[0-9+\-\s\(\)]+$/;
        const invalidNumbers = recipientsArray.filter((phone) => !phoneRegex.test(phone));
