export class TakafulAccidentReportController {
    constructor(createTakafulAccidentReportUseCase, getAllTakafulAccidentReportsUseCase, getTakafulAccidentReportByIdUseCase, deleteTakafulAccidentReportUseCase, getTakafulAccidentReportStatsUseCase) {
    ...
    async addAccidentReport(req, res) {
        try {
            ...
            const createdAccidentReport = await this.createTakafulAccidentReportUseCase.execute(accidentReportData, plateNumber, userId, userName);
            res.status(201).json({
                success: true,
            ...
            console.error("TakafulAccidentReportController addAccidentReport error:", error);
            res.status(400).json({
                success: false,
    ...
    async getAllAccidentReports(req, res) {
        try {
            const accidentReports = await this.getAllTakafulAccidentReportsUseCase.execute();
            res.status(200).json({
                success: true,
                message: "Success",
            ...
            console.error("TakafulAccidentReportController getAllAccidentReports error:", error);
            res.status(500).json({
                success: false,
    ...
    async findById(req, res) {
        try {
            const { id } = req.params;
            const accidentReport = await this.getTakafulAccidentReportByIdUseCase.execute(id);
            res.status(200).json({
                success: true,
                message: "Success",
                data: accidentReport,
    ...
    async deleteAccidentReport(req, res) {
        try {
            ...
            const userId = req.user._id;
            const userName = req.user.name || req.user.email;
            const deletedAccidentReport = await this.deleteTakafulAccidentReportUseCase.execute(id, userId, userName);
            res.status(200).json({
                success: true,
            ...
            console.error("TakafulAccidentReportController deleteAccidentReport error:", error);
            res.status(400).json({
                success: false,
                message: error.message,
            });
    ...
    async getAccidentReportStats(req, res) {
        try {
            ...
            console.error("TakafulAccidentReportController getAccidentReportStats error:", error);
            res.status(500).json({
                success: false,
...
//# sourceMappingURL=TakafulAccidentReportController.js.map