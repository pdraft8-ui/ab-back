export class HolyLandsReportController {
    ...
    async addAccidentReport(req, res) {
        try {
            ...
            const accidentReport = await this.createHolyLandsReportUseCase.execute(accidentData, plateNumber, userId, userName);
            res.status(201).json({
                success: true,
    ...
    async getAllAccidentReports(req, res) {
        try {
            const accidentReports = await this.getAllHolyLandsReportsUseCase.execute();
            res.status(200).json({
                success: true,
    ...
    async findById(req, res) {
        try {
            const { id } = req.params;
            const accidentReport = await this.getHolyLandsReportByIdUseCase.execute(id);
            res.status(200).json({
                success: true,
    ...
    async deleteAccidentReport(req, res) {
        try {
            ...
            const userId = req.user?.id;
            const userName = req.user?.fullName || "Unknown User";
            const result = await this.deleteHolyLandsReportUseCase.execute(id, userId, userName);
            res.status(200).json({
                success: true,
...
//# sourceMappingURL=HolyLandsReportController.js.map