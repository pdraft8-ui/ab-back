...
export const addCustomer = async (req, res, next) => {
    const { first_name, last_name, id_Number, phone_number, joining_date, notes, vehicles, agentsName, city, email, birth_date, } = req.body;
    try {
        ...
        if (req.file) {
            ...
            const result = await localStorageService.upload(req.file, "customers/images");
            imageUrl = result.url;
        }
        else {
            // Default image with full URL
            imageUrl = localStorageService.getFullUrl("/uploads/defaults/default-customer.jpg");
        }
...
export const getAllVehicleInsurances = async (req, res, next) => {
    try {
        ...
        const insurancesWithFullUrls = allInsurances.map((insurance) => ({
            ...insurance,
            insuranceFiles: localStorageService.getFullUrls(insurance.insuranceFiles || []),
        }));
        return res.status(200).json({
...
export const getAllInsurancesData = async (req, res, next) => {
    try {
        const { page = 1, limit = 50, insuranceType, insuranceCategory, status, } = req.query;
        // Build match conditions
        const matchConditions = {};
        if (insuranceType)
            matchConditions.insuranceType = insuranceType;
        if (insuranceCategory)
            matchConditions.insuranceCategory = insuranceCategory;
        if (status)
            matchConditions.policyStatus = status;
        ...
        const paginatedInsurances = insurancesWithFullUrls.slice(startIndex, endIndex);
        ...
        const byInsuranceCategory = allInsurances.reduce((acc, insurance) => {
            const category = insurance.insuranceCategory || insurance.source || "Unknown";
            if (!acc[category])
                acc[category] = [];
...
export const allCustomer = async (req, res, next) => {
    try {
        ...
        const customersWithFullUrls = customerList.map((customer) => convertCustomerToFullUrls(customer));
        return res
            .status(200)
...
export const updateCustomer = async (req, res, next) => {
    const { id } = req.params;
    const { first_name, last_name, id_Number, phone_number, joining_date, notes, city, birth_date, } = req.body;
    try {
        const customer = await Customer.findById(id);
        if (!customer) {
...
export const addVehicle = async (req, res, next) => {
    try {
        ...
        const { plateNumber, model, type, ownership, modelNumber, licenseExpiry, lastTest, color, price, } = req.body;
        let secure_url = "";
        if (req.file) {
...
export const removeVehicle = async (req, res, next) => {
    try {
        ...
        const vehicleToRemove = customer.vehicles.find((v) => v._id.toString() === vehicleId);
        if (!vehicleToRemove)
...
export const addInsuranceToVehicle = async (req, res, next) => {
    const { customerId, vehicleId } = req.params;
    const { insuranceStartDate, insuranceEndDate, insuranceType, insuranceCompany, agent, paymentMethod, insuranceAmount, paidAmount, isUnder24, } = req.body;
    try {
        const customer = await Customer.findById(customerId);
        if (!customer) {
            return res.status(404).json({ message: "Customer not found" });
        ...
        if (req.files && req.files.length > 0) {
            for (const file of req.files) {
                ...
                insuranceFilesUrls.push(`/uploads/Customer/InsuranceFiles/${file.filename}`);
            }
        }
        const newInsurance = {
            insuranceStartDate,
...
export const getInsurancesForVehicle = async (req, res, next) => {
    ...
    try {
        ...
        console.log("Customer vehicles:", customer.vehicles.map((v) => ({
            id: v._id.toString(),
        ...
        if (!vehicle) {
            ...
            const vehicleByString = customer.vehicles.find((v) => v._id.toString() === vehicleId);
            if (vehicleByString) {
...
export const getCustomerInsurances = async (req, res, next) => {
    try {
        ...
        const totalInsurances = allInsurances.length;
        const totalAmount = allInsurances.reduce((sum, ins) => sum + (ins.insuranceAmount || 0), 0);
        const totalPaid = allInsurances.reduce((sum, ins) => sum + (ins.paidAmount || 0), 0);
        const totalRemaining = allInsurances.reduce((sum, ins) => sum + (ins.remainingDebt || 0), 0);
        ...
        const insuranceCompanies = [
            ...new Set(allInsurances.map((ins) => ins.insuranceCompany).filter(Boolean)),
        ];
        const insuranceSubTypes = [
            ...new Set(allInsurances.map((ins) => ins.insuranceSubType).filter(Boolean)),
        ];
        // Get vehicle summary
        const vehicleSummary = {
            totalVehicles: customer.vehicles.length,
            vehiclesWithInsurance: customer.vehicles.filter((v) => v.insurance && v.insurance.length > 0).length,
            vehiclesWithoutInsurance: customer.vehicles.filter((v) => !v.insurance || v.insurance.length === 0).length,
        };
        return res.status(200).json({
...
export const addCustomerInsurance = async (req, res, next) => {
    const { customerId } = req.params;
    const { insuranceType, policyNumber, issueDate, expirationDate, insuranceAmount, premiumAmount, premiumFrequency, premiumPaymentMethod, policyStatus, coverageDetails, insuranceCompany, agent, beneficiaries, notes, } = req.body;
    try {
        const customer = await Customer.findById(customerId);
        if (!customer) {
            return res.status(404).json({ message: "Customer not found" });
        ...
        if (!insuranceType ||
            ...
            return res.status(400).json({
                message: "Missing required fields. Please provide: insuranceType, policyNumber, issueDate, expirationDate, insuranceAmount, premiumAmount, premiumFrequency, premiumPaymentMethod, coverageDetails, insuranceCompany",
            });
        }
...
export const getAllCustomerInsurances = async (req, res, next) => {
    try {
        ...
        const totalPremium = customerInsurances.reduce((sum, ins) => sum + (ins.premiumAmount || 0), 0);
        ...
        Object.keys(insurancesByType).forEach((type) => {
            ...
            insuranceTypeStats[type] = {
                ...
                totalAmount: insurances.reduce((sum, ins) => sum + (ins.insuranceAmount || 0), 0),
                totalPaid: insurances.reduce((sum, ins) => sum + (ins.paidAmount || 0), 0),
                totalRemaining: insurances.reduce((sum, ins) => sum + (ins.remainingDebt || 0), 0),
                totalPremium: insurances.reduce((sum, ins) => sum + (ins.premiumAmount || 0), 0),
                activeCount: insurances.filter((ins) => ins.policyStatus === "Active" || !ins.policyStatus).length,
                expiredCount: insurances.filter((ins) => ins.policyStatus === "Expired")
                    .length,
                suspendedCount: insurances.filter((ins) => ins.policyStatus === "Suspended").length,
                cancelledCount: insurances.filter((ins) => ins.policyStatus === "Cancelled").length,
            };
        });
        return res.status(200).json({
            ...
            summary: {
                ...
                insuranceTypes: insuranceTypes.length,
                insuranceCompanies: insuranceCompanies.length,
                activeInsurances: allInsurances.filter((ins) => ins.policyStatus === "Active" || !ins.policyStatus).length,
                expiredInsurances: allInsurances.filter((ins) => ins.policyStatus === "Expired").length,
                suspendedInsurances: allInsurances.filter((ins) => ins.policyStatus === "Suspended").length,
                cancelledInsurances: allInsurances.filter((ins) => ins.policyStatus === "Cancelled").length,
            },
            insuranceTypes: insuranceTypes,
            insuranceCompanies: insuranceCompanies,
            ...
            if (insurance) {
                ...
                if (updateData.insuranceCompany) {
                    ...
                    if (mongoose.Types.ObjectId.isValid(updateData.insuranceCompany)) {
                        company = await InsuranceCompany.findById(updateData.insuranceCompany);
                    }
                    else {
