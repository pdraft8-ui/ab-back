export class AlMashreqAccidentReportController {
    ...
    async addAccidentReport(req, res) {
        try {
            ...
            const accidentReport = await this.createAlMashreqAccidentReportUseCase.execute(accidentData, plateNumber, userId, userName);
            res.status(201).json({
                success: true,
    ...
    async getAllAccidentReports(req, res) {
        try {
            const accidentReports = await this.getAllAlMashreqAccidentReportsUseCase.execute();
            res.status(200).json({
                success: true,
    ...
    async findById(req, res) {
        try {
            const { id } = req.params;
            const accidentReport = await this.getAlMashreqAccidentReportByIdUseCase.execute(id);
            res.status(200).json({
                success: true,
    ...
    async deleteAccidentReport(req, res) {
        try {
            ...
            const userId = req.user?.id;
            const userName = req.user?.fullName || "Unknown User";
            const result = await this.deleteAlMashreqAccidentReportUseCase.execute(id, userId, userName);
            res.status(200).json({
                success: true,
...
//# sourceMappingURL=AlMashreqAccidentReportController.js.map