...
export class MongoPaymentRepository extends IPaymentRepository {
    ...
    async updatePayment(id, updateData) {
        try {
            const payment = await Payment.findByIdAndUpdate(id, { ...updateData, updatedAt: new Date() }, { new: true }).populate("invoice customer");
            return payment ? this.mapToPaymentEntity(payment) : null;
        }
        catch (error) {
    ...
    async refundPayment(paymentId, refundData) {
        try {
            const payment = await Payment.findByIdAndUpdate(paymentId, {
                ...refundData,
                updatedAt: new Date(),
            }, { new: true }).populate("invoiceId customerId");
            return payment ? this.mapToPaymentEntity(payment) : null;
        }
        catch (error) {
    ...
    async getTranzilaPaymentById(id) {
        try {
            const payment = await TranzilaPayment.findById(id).populate("invoiceId customerId");
    ...
    async getTranzilaPaymentByTransactionId(transactionId) {
        try {
            ...
            throw new Error(`Failed to get Tranzila payment by transaction ID: ${error.message}`);
        }
    }
    async updateTranzilaPayment(id, updateData) {
        try {
            const payment = await TranzilaPayment.findByIdAndUpdate(id, { ...updateData, updatedAt: new Date() }, { new: true }).populate("invoiceId customerId");
            return payment ? this.mapToTranzilaPaymentEntity(payment) : null;
        }
        catch (error) {
