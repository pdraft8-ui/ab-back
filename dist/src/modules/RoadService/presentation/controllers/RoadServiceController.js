export class RoadServiceController {
    constructor(createRoadServiceUseCase, getAllRoadServicesUseCase, getRoadServiceByIdUseCase, updateRoadServiceUseCase, deleteRoadServiceUseCase, getRoadServiceStatsUseCase) {
    ...
    async addRoadService(req, res) {
        try {
            ...
            const createdService = await this.createRoadServiceUseCase.execute(roadServiceData, userId, userName);
            res.status(201).json({
                success: true,
    ...
    async updateRoadService(req, res) {
        try {
            ...
            const userId = req.user._id;
            const userName = req.user.name || req.user.email;
            const updatedService = await this.updateRoadServiceUseCase.execute(id, roadServiceData, userId, userName);
            res.status(200).json({
                success: true,
    ...
    async deleteRoadService(req, res) {
        try {
            ...
            const userId = req.user._id;
            const userName = req.user.name || req.user.email;
            const deletedService = await this.deleteRoadServiceUseCase.execute(id, userId, userName);
            res.status(200).json({
                success: true,
...
//# sourceMappingURL=RoadServiceController.js.map