...
export class UpdateRoadServiceUseCase {
    ...
    async execute(id, roadServiceData, userId, userName) {
        try {
            ...
            if (roadServiceData.amount !== undefined &&
                typeof roadServiceData.amount !== "number") {
                throw new Error("Amount must be a number");
            }
            if (roadServiceData.amountUnder2007 !== undefined &&
                typeof roadServiceData.amountUnder2007 !== "number") {
                throw new Error("Amount under 2007 must be a number");
            }
            // Update the service
            const updatedService = await this.roadServiceRepository.update(id, roadServiceData);
            // Log audit action
            await this.auditService.logAction("UPDATE_ROAD_SERVICE", `Updated road service: ${existingService.companyName}`, userId, userName);
...
//# sourceMappingURL=UpdateRoadServiceUseCase.js.map