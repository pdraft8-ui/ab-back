...
class StatisticsService {
    ...
    async getDashboardStats() {
        try {
            const [totalCustomers, totalIncome, totalExpenses, dueAmount, paymentMethods, totalProfit,] = await Promise.all([
                ...
                this.getCachedData("paymentMethods", () => this.getPaymentMethodStats()),
        ...
        try {
            ...
            if (period === "monthly") {
                ...
                for (let month = 0; month < 12; month++) {
                    const paymentRecord = paymentData.find((p) => p._id.year === currentYear && p._id.month === month + 1);
                    const invoiceRecord = invoiceData.find((i) => i._id.year === currentYear && i._id.month === month + 1);
                    formattedData.push({
            ...
            // Calculate totals
            const totalAmountReceived = formattedData.reduce((sum, item) => sum + item.amountReceived, 0);
            const totalAmountDue = formattedData.reduce((sum, item) => sum + item.amountDue, 0);
            return {
                period: period,
                data: formattedData,
                summary: {
                    ...
                    totalPayments: formattedData.reduce((sum, item) => sum + item.paymentCount, 0),
