...
export class MongoCustomerRepository extends ICustomerRepository {
    ...
    async addVehicleToCustomer(customerId, vehicleData) {
        try {
            const customer = await Customer.findByIdAndUpdate(customerId, { $push: { vehicles: vehicleData } }, { new: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async getVehicleByPlateNumber(plateNumber) {
        try {
            ...
            if (!customer)
                return null;
            const vehicle = customer.vehicles.find((v) => v.plateNumber === plateNumber);
            return vehicle || null;
        }
        catch (error) {
            throw error;
