export class AddVehicleToCustomerUseCase {
    ...
    async execute(customerId, vehicleData, userId, userName) {
        try {
            ...
            const existingVehicle = await this.customerRepository.getVehicleByPlateNumber(vehicleData.plateNumber);
            if (existingVehicle) {
                throw new Error("Vehicle with this plate number already exists");
            }
            // Add vehicle to customer
            const updatedCustomer = await this.customerRepository.addVehicleToCustomer(customerId, vehicleData);
            // Log audit
...
//# sourceMappingURL=AddVehicleToCustomerUseCase.js.map