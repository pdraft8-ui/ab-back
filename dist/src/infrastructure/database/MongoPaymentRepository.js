...
export class MongoPaymentRepository extends IPaymentRepository {
    ...
    async getAllPayments(filters = {}) {
        try {
            const { page = 1, limit = 10, status, customerId, invoiceId, paymentMethod, startDate, endDate, search, } = filters;
            const query = {};
            // Apply filters
            if (status)
                query.status = status;
    ...
    async updatePayment(id, updateData) {
        try {
            const payment = await Payment.findByIdAndUpdate(id, { ...updateData, updatedAt: new Date() }, { new: true })
                .populate("invoice", "invoiceNumber balanceDue")
                .populate("customer", "name email phone")
                .populate("createdBy", "name")
                .populate("updatedBy", "name");
    ...
    async getAllTranzilaPayments(filters = {}) {
        try {
            const { page = 1, limit = 10, status, customerId, invoiceId, startDate, endDate, } = filters;
            const query = {};
            if (status)
                query.status = status;
            if (customerId)
    ...
    mapToEntity(paymentDoc) {
        ...
        return new PaymentEntity({
            ...
            invoiceId: paymentDoc.invoice?._id?.toString() || paymentDoc.invoice?.toString(),
            customerId: paymentDoc.customer?._id?.toString() || paymentDoc.customer?.toString(),
            ...
            createdBy: paymentDoc.createdBy?._id?.toString() ||
                paymentDoc.createdBy?.toString(),
            updatedBy: paymentDoc.updatedBy?._id?.toString() ||
