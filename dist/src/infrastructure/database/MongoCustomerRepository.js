...
export class MongoCustomerRepository extends ICustomerRepository {
    ...
    async getCustomerById(id) {
        try {
            const customer = await Customer.findById(id).populate("agentsId", "first_name last_name");
    ...
    async getAllCustomers(filters = {}) {
        try {
            const { page = 1, limit = 10, search, city, agentId, sortBy = "createdAt", sortOrder = "desc", } = filters;
            let query = {};
    ...
    async getCustomerByIdNumber(idNumber) {
        try {
            const customer = await Customer.findOne({ id_Number: idNumber }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async getCustomersByAgent(agentId) {
        try {
            const customers = await Customer.find({ agentsId: agentId }).populate("agentsId", "first_name last_name");
    ...
    async addVehicleToCustomer(customerId, vehicleData) {
        try {
            const customer = await Customer.findByIdAndUpdate(customerId, { $push: { vehicles: vehicleData } }, { new: true, runValidators: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async removeVehicleFromCustomer(customerId, vehicleId) {
        try {
            const customer = await Customer.findByIdAndUpdate(customerId, { $pull: { vehicles: { _id: vehicleId } } }, { new: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async updateVehicle(customerId, vehicleId, updateData) {
        try {
            const customer = await Customer.findOneAndUpdate({ _id: customerId, "vehicles._id": vehicleId }, { $set: { "vehicles.$": { ...updateData, _id: vehicleId } } }, { new: true, runValidators: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async getCustomerVehicles(customerId) {
        try {
            const customer = await Customer.findById(customerId).populate("agentsId", "first_name last_name");
    ...
    async addInsuranceToVehicle(customerId, vehicleId, insuranceData) {
        try {
            const customer = await Customer.findOneAndUpdate({ _id: customerId, "vehicles._id": vehicleId }, { $push: { "vehicles.$.insurance": insuranceData } }, { new: true, runValidators: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async removeInsuranceFromVehicle(customerId, vehicleId, insuranceId) {
        try {
            const customer = await Customer.findOneAndUpdate({ _id: customerId, "vehicles._id": vehicleId }, { $pull: { "vehicles.$.insurance": { _id: insuranceId } } }, { new: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async updateVehicleInsurance(customerId, vehicleId, insuranceId, updateData) {
        try {
            ...
            }, { new: true, runValidators: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async getAllInsurancesData() {
        try {
            const customers = await Customer.find({}).populate("agentsId", "first_name last_name");
    ...
    async addCustomerInsurance(customerId, insuranceData) {
        try {
            const customer = await Customer.findByIdAndUpdate(customerId, { $push: { insurances: insuranceData } }, { new: true, runValidators: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async removeCustomerInsurance(customerId, insuranceId) {
        try {
            const customer = await Customer.findByIdAndUpdate(customerId, { $pull: { insurances: { _id: insuranceId } } }, { new: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async updateCustomerInsurance(customerId, insuranceId, updateData) {
        try {
            const customer = await Customer.findOneAndUpdate({ _id: customerId, "insurances._id": insuranceId }, { $set: { "insurances.$": { ...updateData, _id: insuranceId } } }, { new: true, runValidators: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async getInsuranceById(insuranceId) {
        try {
            ...
            if (!customer)
                return null;
            const insurance = customer.insurances.find((i) => i._id.toString() === insuranceId);
            return insurance
                ? {
                    ...insurance.toObject(),
                    customerName: `${customer.first_name} ${customer.last_name}`,
    ...
    async deleteInsuranceById(insuranceId) {
        try {
            const customer = await Customer.findOneAndUpdate({ "insurances._id": insuranceId }, { $pull: { insurances: { _id: insuranceId } } }, { new: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async addCheckToInsurance(customerId, vehicleId, insuranceId, checkData) {
        try {
            const customer = await Customer.findOneAndUpdate({
                ...
                "vehicles._id": vehicleId,
                "vehicles.insurance._id": insuranceId,
            }, { $push: { "vehicles.$.insurance.$.checkDetails": checkData } }, { new: true, runValidators: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async removeCheckFromInsurance(customerId, vehicleId, checkId) {
        try {
            const customer = await Customer.findOneAndUpdate({ _id: customerId, "vehicles._id": vehicleId }, { $pull: { "vehicles.$.insurance.$.checkDetails": { _id: checkId } } }, { new: true }).populate("agentsId", "first_name last_name");
            return customer ? this.mapToEntity(customer) : null;
        }
        catch (error) {
            throw error;
    ...
    async getInsuranceChecks(customerId, vehicleId, insuranceId) {
        try {
            ...
            if (!vehicle)
                return null;
            const insurance = vehicle.insurance.find((i) => i._id.toString() === insuranceId);
...
//# sourceMappingURL=MongoCustomerRepository.js.map