    ...
    async execute(paymentId, refundData, userId, userName) {
        try {
            ...
            if (refundData.refundAmount > paymentEntity.paymentAmount) {
                throw new Error(`Refund amount (${refundData.refundAmount}) cannot exceed original payment amount (${paymentEntity.paymentAmount})`);
            }
            // Get invoice for balance update
            const invoice = await this.invoiceRepository.getInvoiceById(paymentEntity.invoiceId);
            if (!invoice) {
            ...
            const updatedPayment = await this.paymentRepository.updatePayment(paymentId, paymentEntity.toJSON());
            // Update invoice balance (add back the refunded amount)
            const currentBalance = await this.invoiceRepository.getInvoiceBalance(paymentEntity.invoiceId);
            const newBalance = currentBalance + refundData.refundAmount;
            await this.invoiceRepository.updateInvoiceBalance(paymentEntity.invoiceId, newBalance);
            // Send notification
