...
export class CreateTranzilaPaymentUseCase {
    ...
    async execute(paymentData, userId) {
        try {
            ...
            const customer = await this.customerRepository.findById(paymentData.customerId);
            if (!customer) {
                throw new Error("Customer not found");
            }
            // 4. Validate payment amount
            if (paymentData.amount > invoice.balanceDue) {
                throw new Error(`Payment amount (${paymentData.amount}) exceeds remaining balance (${invoice.balanceDue})`);
            }
            ...
            const tranzilaPayment = new TranzilaPayment({
                ...
                cancelUrl: paymentData.cancelUrl || `${process.env.FRONTEND_URL}/payment/cancel`,
                createdBy: userId,
                status: "pending",
            });
            ...
            // 7. Save to database
            const savedPayment = await this.paymentRepository.createTranzilaPayment(tranzilaPayment);
            // 8. Create payment with Tranzila
            const tranzilaRequest = this.buildTranzilaRequest(savedPayment, invoice, customer);
            const tranzilaResponse = await this.tranzilaService.createPayment(tranzilaRequest);
            ...
            if (tranzilaResponse.success) {
                ...
                await this.notificationService.sendPaymentCreatedNotification(savedPayment, invoice, customer);
                return {
                    success: true,
