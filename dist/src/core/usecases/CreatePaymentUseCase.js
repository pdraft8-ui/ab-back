...
export class CreatePaymentUseCase {
    ...
    async execute(paymentData, userId, userName) {
        try {
            ...
            const currentBalance = await this.invoiceRepository.getInvoiceBalance(paymentData.invoiceId);
            if (paymentData.paymentAmount > currentBalance) {
                throw new Error(`Payment amount (${paymentData.paymentAmount}) exceeds remaining balance (${currentBalance})`);
            }
            if (currentBalance <= 0) {
            ...
            const savedPayment = await this.paymentRepository.createPayment(payment.toJSON());
            // Update invoice balance
            const newBalance = Math.max(0, currentBalance - paymentData.paymentAmount);
            await this.invoiceRepository.updateInvoiceBalance(paymentData.invoiceId, newBalance);
            // Send notification
