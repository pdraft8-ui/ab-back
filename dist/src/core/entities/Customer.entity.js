export class Customer {
    ...
    getInsuranceByPolicyNumber(policyNumber) {
        return this.insurances.find((insurance) => insurance.policyNumber === policyNumber);
    }
    addVehicle(vehicle) {
        if (!this.vehicles)
            this.vehicles = [];
    ...
    removeVehicle(vehicleId) {
        if (!this.vehicles)
            return false;
        const index = this.vehicles.findIndex((v) => v._id?.toString() === vehicleId?.toString());
        if (index > -1) {
    ...
    removeInsurance(insuranceId) {
        if (!this.insurances)
            return false;
        const index = this.insurances.findIndex((i) => i._id?.toString() === insuranceId?.toString());
        if (index > -1) {
