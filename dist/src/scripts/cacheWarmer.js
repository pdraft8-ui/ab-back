...
class CacheWarmer {
    ...
    async warmCustomerCache() {
        try {
            ...
            // Cache customer list
            await this.cacheService.set("customers:list", customers.map((c) => ({
                id: c._id,
    ...
    async warmChequeCache() {
        try {
            ...
            for (const batch of batches) {
                await Promise.all(batch.map(async (cheque) => {
    ...
    async warmApiResponseCache() {
        try {
            ...
            for (const endpoint of commonEndpoints) {
                ...
                await this.cacheService.set(cacheKey, { cached: true, timestamp: Date.now() }, this.config.ttl);
            }
    ...
    async warmDashboardCache() {
        try {
            ...
            await this.cacheService.set("dashboard:stats", dashboardStats, this.config.ttl);
